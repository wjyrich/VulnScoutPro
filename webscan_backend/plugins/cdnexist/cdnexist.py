...
# 国内外常见的cdn段, Reference:https://www.cnblogs.com/L1079991001/p/10300628.html
# 根据阿里与腾讯的CDN导入了部分IP段
...
def iscdn(host):
    """
        host的主机IP是否为CDN的IP
        是则返回true，不是cdn返false，及没有CDN，是源IP
    """
    result = False
    ...
    for cdn in cdns:
        if ipaddress.ip_address(host) in ipaddress.ip_network(cdn):
            result = True
    # 跟据IP获取ASN，判断是否在CDN所在ASN中
    try:
        with geoip2.database.Reader(os.path.dirname(__file__) + '/../../database/GeoLite2-ASN.mmdb') as reader:  # 数据库更新地址 https://geolite.maxmind.com/download/geoip/database/GeoLite2-ASN.tar.gz
...
if __name__ == '__main__':
    # 未使用CDN
    print(iscdn('https://github.com/wjyrich'))
    # 使用了CDN
    print(iscdn('https://github.com/wjyrich'))